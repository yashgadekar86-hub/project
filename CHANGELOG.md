...
execution bridge for Windows. Upstream behaviour for stocks/crypto is unchanged.

### Added
- `tradingagents/brokers/` — new package: `MT5Broker` (connection, gold-symbol
  auto-detect, risk-based lot sizing, SL/TP, position flattening) and
  `GoldExecutor` (maps the Portfolio Manager rating onto MT5 orders). Dry-run by
  default; real orders require both `MT5_DRY_RUN=false` and `--live`.
- `tradingagents/gold_config.py` — gold overlays: macro news queries tuned to gold
  drivers, MT5 gold-symbol candidate list, `is_gold_symbol` helper.
- `gold_mt5.py` — runner: analyse gold, save reports, and execute on MT5.
- New `commodity` asset type; gold symbols (`XAUUSD`, `GOLD`, `GC=F`, …) classify
  as commodity instead of stock.
- Commodity-mode Fundamentals Analyst: uses FRED macro series + macro news (real
  yields, dollar, CPI, Fed funds, gold fix, central-bank/ETF flows) instead of
  company financials, which do not exist for a metal.
- FRED macro series: `gold_price` (LBMA PM fix), `real_yield_5y/10y/20y/30y`,
  `tips_10y`, `dxy`.
- `docs/GOLD_MT5_WINDOWS.md` — full Windows setup guide.
- `[mt5]` pip extra (`MetaTrader5>=5.0.45`, Windows-only wheel).
- `tests/test_gold_mt5.py` — gold + MT5 test suite.

### Changed
- Instrument context tells agents the target is a commodity (no earnings/balance
  sheets), so they never hallucinate company fundamentals for gold.
- `detect_asset_type` classifies futures/commodities before crypto/stock.

### Safety
- Default is **dry-run**: the pipeline computes the exact order but sends nothing.
- Bearish signals flatten an existing long; they open a short only with
  `--allow-short`. `REVIEW`/unparseable decisions are never traded.
