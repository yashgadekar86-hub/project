...
# Futures canonical symbols (Yahoo ``=F``) resolve through the commodity
# pipeline: the fundamentals analyst becomes a macro/supply-demand analyst
# because futures have no company financials. Covers gold (GC=F), silver
# (SI=F), oil (CL=F, BZ=F), nat gas (NG=F), copper (HG=F), etc.
COMMODITY_SUFFIXES = ("=F",)

# Canonical commodity symbols that do not carry the ``=F`` suffix.
COMMODITY_SYMBOLS = frozenset({"GC=F", "SI=F", "PL=F", "PA=F", "CL=F", "BZ=F", "NG=F", "HG=F"})
...
def detect_asset_type(ticker: str) -> AssetType:
    ...
    Commodities/futures are classified before crypto: ``XAUUSD`` normalizes to
    ``GC=F`` (COMEX gold future) and must read as commodity, not fall through
    to the stock pipeline and hallucinate company fundamentals.
    """
    canonical = normalize_ticker_symbol(ticker)
    if canonical in COMMODITY_SYMBOLS or canonical.endswith(COMMODITY_SUFFIXES):
